...
Løsningenformlen fra {prf:ref}`loesningandengrad` lød
...
Vi starter med at vise resultatet for $d<0$ (nul rødder): Antag $d<0$. Af {prf:ref}`ligeoguligeroedder` fra {numref}`Kapitel %s<roedder>` ved vi, at hvis radikanden er negativ, så findes roden ikke. Ergo har vi, at 
