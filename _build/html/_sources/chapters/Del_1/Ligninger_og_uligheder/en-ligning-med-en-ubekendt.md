...
Ligningen fra {prf:ref}`enligningenubekendteks1` kaldes en simpel ligning, og kan reduceres ned til formen
...
Korollaret er et resultat af {prf:ref}`loesningfoerstegrad`, hvor vi vælger $b$ på en bestemt måde.
 
```{prf:bevis}
:class: text

Lad $b=\tilde{b} - c$ i {prf:ref}`loesningfoerstegrad`, og brug resultatet. 
